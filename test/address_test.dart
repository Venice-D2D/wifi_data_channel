  ...
  group('retrieveHotspotIPAddress', () {
    test('should find new address', () {
      ...
      NetworkInterface ni2 =
          TestNetworkInterface("v4-rmnet_data2", ['192.0.0.4']);
      NetworkInterface hotspot = TestNetworkInterface("wlan", [hotspotAddress]);

      InternetAddress result = canal.retrieveHotspotIPAddress(
          [ni1, hotspot, ni2],
    ...
    test('should not find address without second parameter', () {
      ...
      expect(
          () => canal.retrieveHotspotIPAddress([ni1, hotspot, ni2]),
          throwsA(predicate((e) =>
              e is StateError &&
              e.message ==
    ...
    test('should find the new address among private IPs', () {
      ...
      NetworkInterface ni1 = TestNetworkInterface("wlan0", [hotspotAddress]);
      NetworkInterface ni2 =
          TestNetworkInterface("v4-rmnet_data2", ['172.18.42.15']);
      NetworkInterface ni3 =
          TestNetworkInterface("test-interface", ['192.168.1.15']);

      InternetAddress result = canal
          .retrieveHotspotIPAddress([ni1, ni2, ni3], oldInterfaces: [ni2, ni3]);
      expect(result.address, hotspotAddress);
    });

    test('should throw when given an empty interfaces list', () {
      expect(
          () => canal.retrieveHotspotIPAddress([]),
          throwsA(predicate((e) =>
              e is ArgumentError &&
              e.message ==
